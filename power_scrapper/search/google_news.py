...
def build_google_news_url(
    ...
    *,
    ...
    """Build a Google News tab URL from query and config."""
    params = f"q={quote_plus(query)}&hl={config.language}&gl={config.country}&tbm=nws&start={start}"
...
class GoogleNewsStrategy(ISearchStrategy):
        ...
        *,
        ...
        for page_num in range(max_pages):
            ...
            try:
                ...
                if not articles:
                    logger.info(
                        "GoogleNews page %d returned 0 results, stopping", page_num + 1
                    )
                    break
